class QuestionMailer < Mailer
  ...
  def asked_question(journal)
    ...
    from = question.author ? "#{question.author.name} (Redmine) <#{Setting.mail_from}>" : nil
  ...
  def answered_question(question, closing_journal)
    from = question.assigned_to ? "#{question.assigned_to.name} (Redmine) <#{Setting.mail_from}>" : nil
